...
var minWidth = 1025,
    ...
    lastScroll = round(scroll/16) * 160;
...
$(function() {
    $("html, body").mousewheel(function(event, delta) {
        ...
        var fullWidth = $(window),

            delta = event.originalEvent.wheelDelta * deltaWebkit || event.originalEvent.deltaY * deltaFirefox,
            deltaLimit = Math.min(Math.max((delta), -8), 8),
            scrollTop = fullWidth.scrollLeft(),
            finalScroll = scrollTop - parseInt( deltaLimit * scrollDistance );

            TweenMax.to(fullWidth, scrollTime, {
	...
	handleEvents: function() {
		var self = this,
            ...
            handleScroll = function() {
				...
				if (scrollLeft === 0) {
					$(window).unbind('scroll.stellarsite').bind('scroll.stellarsite', debounce(handleScroll, 0));
				} 
                else {
					$('html,body').animate({
		...
		if (window.location.href.indexOf('#show-offset-parents-default') === -1) {
			$(window).bind('scroll.stellarsite', debounce(handleScroll, scrollTimeout));
		}
	}
};
parallaxScroll.init();
